...
import { Mail, MapPin, MessageSquare, Phone, Store, Users, Instagram, Twitter, Linkedin } from "lucide-react";
          ...
          <div className="max-w-2xl mx-auto rounded-3xl border border-white/8 bg-gradient-to-br from-white/[0.04] to-transparent p-6 sm:p-10">
            ...
            >
              <div className="grid sm:grid-cols-2 gap-5">
                ...
                {tab === "general" && <Field label="Topic" name="topic" />}
                {tab !== "general" && <Field label="Restaurant name" name="restaurant" />}
                {tab !== "general" && <Field label="City" name="city" />}
                {tab === "onboarding" && <Field label="Number of locations" name="locations" type="number" />}
              </div>
              <Field
                label="Message"
                name="message"
              ...
              <button
                type="submit"
                className="inline-flex items-center gap-2 rounded-2xl lime-gradient text-ink px-6 py-3 text-sm font-semibold hover:opacity-90 transition-opacity"
              >
...
function Field({
  label, name, type = "text", textarea = false, placeholder,
}: {
    ...
    <label className="block">
      ...
      {textarea ? (
        <textarea
          ...
          className="mt-2 w-full rounded-xl border border-white/10 bg-white/[0.04] px-4 py-3 text-sm text-white placeholder:text-ink-soft focus:border-primary/60 focus:ring-4 focus:ring-primary/15 outline-none transition-all"
        />
      ) : (
        <input
